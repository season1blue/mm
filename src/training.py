...
def fine_tune(epoch,
    ...
    for i, batch in tqdm(enumerate(train_loader), desc="Train", total=len(train_loader), ncols=80):
        # Forward pass
        if args.task == 'twitter_ae':
            ...
            aesc_infos = {key: value for key, value in batch['AESC'].items()}  # dict_item{ mask, label, spans }
