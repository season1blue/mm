...
def main(rank, args):
    ...
    # logger.info('Initialed with {} GPU(s)'.format(args.gpu_num))
    ...
    while epoch < args.epochs:
        ...
        if (epoch + 1) % args.eval_every == 0:
            ...
            logger.warning('DEV  aesc_p:{}   aesc_r:{}   aesc_f:{}'.format(
                res_dev['aesc_pre'], res_dev['aesc_rec'], res_dev['aesc_f']))

            logger.warning('TEST  aesc_p:{}   aesc_r:{}   aesc_f:{}'.format(
    ...
    logger.warning("-"*80)
    logger.warning("Total traing: " + str(datetime.now() - start))

    logger.warning('BEST DEV   aesc_p:{}   aesc_r:{}   aesc_f:{}'.format(
        best_dev_res['aesc_pre'], best_dev_res['aesc_rec'], best_dev_res['aesc_f']))

    logger.warning('BEST DEV--TEST   aesc_p:{}   aesc_r:{}   aesc_f:{}'.format(
        best_dev_test_res['aesc_pre'], best_dev_test_res['aesc_rec'], best_dev_test_res['aesc_f']))

    logger.warning('BEST TEST   aesc_p:{}   aesc_r:{}   aesc_f:{}'.format(
        best_test_res['aesc_pre'], best_test_res['aesc_rec'], best_test_res['aesc_f']))
